...
export class ArticlesComponent implements OnInit {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.articlesService.articles().subscribe((data: any) => {
          ...
          // this.filterStatusLoadPage(this.paramsUrl())
        ...
        },
        {
          ...
          render: (data, type, full) => {
            ...
            if (data.status == 'eliminado') {
              return `<button class=" mb-1 btnDelete btn btn-sm btn-danger" id=${data.id}> <i class="fas fa-times" ></i> </button> <button class=" mb-1 btnEnable btn btn-sm btn-blue" id=${data.id}> <i class="fas fa-check" ></i> </button> <button class=" mb-1 btnGo btn btn-sm btn-white" id=${data.id}> <i style="font-size:95%" class="fas fa-eye" ></i> </button>`
            } else {
  ...
  apllyFilters(recentLoad:any) {
    ...
    this.datatableElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      if(this.filterCategory?.length){
        dtInstance.search('').column(3).search("^"+this.filterStatus+"", true, false, true).column(4).search("^"+this.filterCategory+"$", true, false, true).page(this.lastPageArticles - 1).draw(draw);
      }else{
        dtInstance.search('').column(3).search("^"+this.filterStatus+"", true, false, true).column(4).search("^"+this.filterCategory+"", true, false, true).page(this.lastPageArticles - 1).draw(draw);
      }
  ...
  // paramsUrl(){
  //   let filterStatusActive = this.activatedRoute.snapshot.queryParamMap.get('status');
  //   if (filterStatusActive?.length) {
  //     if(filterStatusActive == 'publicado' || filterStatusActive == 'no-publicado' || filterStatusActive == 'eliminado'){
  //       this.filterStatusActive = filterStatusActive;
  //       return filterStatusActive;
  //     }
  //     return '';
  //   }else if(localStorage.getItem('statusArticle')?.length){
  //     this.filterStatusActive = localStorage.getItem('statusArticle');
  //   }
  //     return '';
  // }
  // deshabilitar
  ...
  //   filterStatus(text:string){
  //     localStorage.setItem('lastPageArticles','1');
  //     this.filterStatusActive = text;
  //     if(text?.length){
  //       this.location.go('administrar/articulos?status='+text);
  //     }else{
  //       this.location.go('administrar/articulos');
  //     }
  //     localStorage.setItem('statusArticle',text);

  //    if(text == 'no-publicado'){
  //     text = "No publicado";
  //    }

  //   this.datatableElement.dtInstance.then((dtInstance: DataTables.Api) => {
  //     if(text?.length){
  //       dtInstance.column(3).search("^"+text+"", true, false, true).column(4).search("^"+this.filterCategory+"", true, false, true).draw();
  //     }else{
  //       dtInstance.column(3).search("publicado").draw(false);
  //     }

  //   });
  // }

  // filterStatusLoadPage(text:any){

  //   if(text?.length){
  //     this.filterStatusActive = text;
  //   }else if(localStorage.getItem('statusArticle')?.length){
  //     this.filterStatusActive = localStorage.getItem('statusArticle');
  //     text = localStorage.getItem('statusArticle');
  //     this.location.go('administrar/articulos?status='+text);
  //   }
  ...
  //   // se resta por que cuenta páginas desde el 0
  //   let lastPage = this.lastPage() -1;
  //   this.datatableElement.dtInstance.then((dtInstance: DataTables.Api) => {
  //     if(text?.length){
  //       dtInstance.column(3).search("^"+text+"", true, false, true).page(lastPage).draw(false);
  //     }else{
  //       dtInstance.column(3).search("publicado").page(lastPage).draw(false);
  //     }
